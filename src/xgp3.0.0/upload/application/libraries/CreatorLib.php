...
class CreatorLib extends XGPCore
{
     ...
     * @param boolean $home_world Home world
     *
     * @return void
     */
    private function returnSize($position, $home_world = false)
    {
        if (!$home_world) {

            // THIS DIAMETERS ARE CALCULATED TO RETURN THE CORRECT AMOUNT OF FIELDS, IT SHOULD WORK AS OGAME.
            $min    = [
                9747,  9849,  9900, 11091, 12166,
                12166, 11875, 12962, 12689, 12410,
                12084, 11662, 10441, 9000, 8063
            ];

            $max    = [
                10393, 10489, 11705, 14248, 14595,
                15067, 15395, 15685, 15428, 14934,
                14283, 13077, 11000, 9644, 8603
            ];
     ...
     * @param int $position        Position
     * @param int $planet_owner_id Planet owner Id
     * @param string $planet_name  Planet name
     * @param boolean $home_world  Main planet
     *
     * @return boolean
     */
    public function createPlanet($galaxy, $system, $position, $planet_owner_id, $planet_name = '', $home_world = false)
    {
        ...
        if (!$planet_exist) {
            ...
            $planet                             = $this->returnSize($position, $home_world);
            ...
            $planet['planet_metal_perhour']     = FunctionsLib::readConfig('metal_basic_income');
            $planet['planet_crystal_perhour']   = FunctionsLib::readConfig('crystal_basic_income');
            $planet['planet_deuterium_perhour'] = FunctionsLib::readConfig('deuterium_basic_income');
            ...
            $planet['planet_name']          = ($planet_name == '') ? $this->langs['ge_colony'] : $planet_name;

            if ($home_world == true) {
                $planet['planet_name'] = $this->langs['ge_home_planet'];
            }
            ...
            parent::$db->query("INSERT INTO " . BUILDINGS . " SET`building_planet_id` = '" . $last_id . "';");
            parent::$db->query("INSERT INTO " . DEFENSES . " SET `defense_planet_id` = '" . $last_id . "';");
            parent::$db->query("INSERT INTO " . SHIPS . " SET `ship_planet_id` = '" . $last_id . "';");
     ...
     * @param string $moon_name Moon name
     * @param int $chance       Chance
     * @param int $size         Size
     *
     * @return string
     */
    public function createMoon($galaxy, $system, $planet, $owner, $moon_name = '', $chance = '', $size = '')
    {
        $planet_name    = '';
        ...
        if ($MoonPlanet['id_moon'] == '' && $MoonPlanet['planet_id'] != 0) {
            ...
            parent::$db->query(
                "INSERT INTO " . PLANETS . " SET
                `planet_name` = '". (($moon_name == '') ? $this->langs['fcm_moon'] : $moon_name) ."',
