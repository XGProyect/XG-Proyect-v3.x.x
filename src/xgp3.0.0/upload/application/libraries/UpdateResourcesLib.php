...
{
    public static function updateResource(&$current_user, &$current_planet, $UpdateTime, $Simul = false)
    {
        ...
        $current_planet['planet_metal_max']     = ProductionLib::max_storable($current_planet[$resource[22]]);
        $current_planet['planet_crystal_max']   = ProductionLib::max_storable($current_planet[$resource[23]]);
        $current_planet['planet_deuterium_max'] = ProductionLib::max_storable($current_planet[$resource[24]]);
        ...
        $post_porcent                       = ProductionLib::max_production(
        ...
        foreach ($ProdGrid as $ProdID => $formula) {
            ...
            // BOOST
            $geologe_boost      = 1 + ( 1 * ( Officiers_Lib::is_officier_active(
                $current_user['premium_officier_geologist']
            ) ? GEOLOGUE : 0));
            $engineer_boost     = 1 + ( 1 * ( Officiers_Lib::is_officier_active(
            ...
            // PRODUCTION
            $Caps['planet_metal_perhour']       += ProductionLib::current_production(
                ProductionLib::production_amount($metal_prod, $geologe_boost),
                $post_porcent
            );

            $Caps['planet_crystal_perhour']     += ProductionLib::current_production(
                ProductionLib::production_amount($crystal_prod, $geologe_boost),
                $post_porcent
            );

            $Caps['planet_deuterium_perhour']   += ProductionLib::current_production(
                ProductionLib::production_amount($deuterium_prod, $geologe_boost),
                $post_porcent
            );
            ...
            if ($ProdID >= 4) {
                ...
                $Caps['planet_energy_max']  += ProductionLib::production_amount($energy_prod, $engineer_boost, true);
            } else {

                $Caps['planet_energy_used'] += ProductionLib::production_amount($energy_prod, 1, true);
            }
        }
