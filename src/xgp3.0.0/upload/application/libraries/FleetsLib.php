...
class FleetsLib extends XGPCore
{
    ...
    public static function fleet_resources_popup($fleet_row, $text, $fleet_type)
    {
        ...
        if ($total_resources != 0) {
            ...
            $resources_popup  .= '<tr><td width=50% align=left><font color=white>' . parent::$lang['Metal'] . '<font></td><td width=50% align=right><font color=white>' . FormatLib::pretty_number($fleet_row['fleet_resource_metal']) . '<font></td></tr>';
            $resources_popup  .= '<tr><td width=50% align=left><font color=white>' . parent::$lang['Crystal'] . '<font></td><td width=50% align=right><font color=white>' . FormatLib::pretty_number($fleet_row['fleet_resource_crystal']) . '<font></td></tr>';
            $resources_popup  .= '<tr><td width=50% align=left><font color=white>' . parent::$lang['Deuterium'] . '<font></td><td width=50% align=right><font color=white>' . FormatLib::pretty_number($fleet_row['fleet_resource_deuterium']) . '<font></td></tr>';
    ...
    public static function fleet_ships_popup($fleet_row, $text, $fleet_type, $current_user = '')
    {
        ...
        $espionage_tech = OfficiersLib::getMaxEspionage($current_user['research_espionage_technology'], $current_user['premium_officier_technocrat']);
        ...
        if ($espionage_tech < 2 && $fleet_row['fleet_owner'] != $current_user['user_id']) {
            ...
            foreach ($ships as $item => $group) {
                if ($group != '') {
                    ...
                    if ($fleet_row['fleet_owner'] == $current_user['user_id']) {
                        $pop_up .= '<tr><td width=50% align=left><font color=white>' . parent::$lang['tech'][$ship[0]] . ':<font></td><td width=50% align=right><font color=white>' . FormatLib::pretty_number($ship[1]) . '<font></td></tr>';
    ...
    {
        $enemy_name = parent::$db->queryFetch('SELECT `user_name`
                                                                                                    FROM ' . USERS . "
    ...
    public static function flying_fleets_table($fleet_row, $Status, $Owner, $Label, $Record, $current_user)
    {
        ...
        if ($Status != 2) {
            ...
            $StartID .= self::start_link($fleet_row, $FleetPrefix . $FleetStyle[$MissionType]);
            ...
            $TargetID .= self::target_link($fleet_row, $FleetPrefix . $FleetStyle[ $MissionType ]);
        } else {
            if ($StartType == 1) {
