...
class Overview extends XGPCore
{
    ...
    private function get_messages()
    {
        ...
        if ($this->_current_user['new_message'] != 0) {
            ...
            if ($this->_current_user['new_message'] == 1) {
                $new_message   .= '<th colspan="4">' . FunctionsLib::set_url('game.php?page=messages', $this->_lang['ov_have_new_message'], $this->_lang['ov_have_new_message']) . '</th>';
            }

            if ($this->_current_user['new_message'] > 1) {
                $link_text = str_replace('%m', FormatLib::pretty_number($this->_current_user['new_message']), $this->_lang['ov_have_new_messages']);
                $new_message   .= '<th colspan="4">' . FunctionsLib::set_url('game.php?page=messages', $link_text, $link_text) . '</th>';
            }
    ...
    private function get_fleet_movements()
    {
        ...
        while ($fleets = parent::$db->fetchArray($own_fleets)) {
            ...
            if ($fleets['fleet_owner'] == $this->_current_user['user_id']) {
                ...
                if ($start_time > time()) {
                    $fleet_row[$start_time . $id] = FleetsLib::flying_fleets_table($fleets, 0, true, $label, $record, $this->_current_user);
                }
                ...
                if (($fleets['fleet_mission'] != 4) && ($fleets['fleet_mission'] != 10)) {
                    ...
                    if ($stay_time > time()) {
                        $fleet_row[$stay_time . $id] = FleetsLib::flying_fleets_table($fleets, 1, true, $label, $record, $this->_current_user);
                    }

                    $label = 'fe';

                    if ($end_time > time()) {
                        $fleet_row[$end_time . $id] = FleetsLib::flying_fleets_table($fleets, 2, true, $label, $record, $this->_current_user);
                    }
                }

                if ($fleets['fleet_mission'] == 4 && $start_time < time() && $end_time > time()) {
                    $fleet_row[$end_time . $id] = FleetsLib::flying_fleets_table($fleets, 2, true, 'none', $record, $this->_current_user);
                }
            }
                    ...
                    if ($fleets['fleet_owner'] != $this->_current_user['user_id']) {
                        if ($fleets['fleet_mission'] == 2) {
                            ...
                            if ($start_time > time()) {
                                $fleet_row[$start_time . $id] = FleetsLib::flying_fleets_table($fleets, 0, false, 'ofs', $record1, $this->_current_user);
                            }
                        }
                        ...
                        if (($fleets['fleet_mission'] == 1) && ($fleet_group > 0)) {
                            ...
                            if ($start_time > time()) {
                                $fleet_row[$start_time . $id] = FleetsLib::flying_fleets_table($fleets, 0, false, 'ofs', $record, $this->_current_user);
                            }
                        }
                    }
                    ...
                    if ($fleets['fleet_owner'] != $this->_current_user['user_id']) {
                        if ($fleets['fleet_mission'] != 8) {
                            ...
                            if ($fleets['fleet_mission'] == 5) {
                                if ($stay_time > time()) {
                                    $fleet_row[$stay_time . $id] = FleetsLib::flying_fleets_table($fleets, 1, false, 'oft', $record, $this->_current_user);
                                }
                            }
                        }
                    }
    ...
    private function get_planet_moon()
    {
        ...
        if ($this->_current_planet['moon_id'] != 0 && $this->_current_planet['moon_destruyed'] == 0 && $this->_current_planet['planet_type'] == 1) {
            ...
            $return['moon_img'] = FunctionsLib::set_url($url, $moon_name, FunctionsLib::set_image($image, $moon_name, $attributes));
