    ...
    class Trader extends XGPCore
    {
        ...
        private function build_page()
        {
            ...
            if ($this->_current_user['premium_dark_matter'] < $this->_tr_dark_matter) {
                FunctionsLib::message(str_replace('%s', $this->_tr_dark_matter, $this->_lang['tr_darkmatter_needed']),
                    '', '', true);
                die();
            }
                    ...
                    case 'cristal': {
                        if ($_POST['metal'] < 0 or $_POST['deut'] < 0) {
                            ...
                            if ($this->_current_planet['planet_crystal'] > $necessaire) {
                                ...
                                FunctionsLib::message($this->_lang['tr_not_enought_crystal'], "game.php?page=trader",
                                    1);
                            }
                        }
                        break;
                    ...
                    case 'deuterium': {
                        if ($_POST['cristal'] < 0 or $_POST['metal'] < 0) {
                            ...
                            if ($this->_current_planet['planet_deuterium'] > $necessaire) {
                                ...
                                FunctionsLib::message($this->_lang['tr_not_enought_deuterium'], "game.php?page=trader",
                                    1);
                            }
                        }
                        break;
        ...
        public function check_storage($amount, $force = null)
        {
            ...
            if ($check['metal'] === true && $check['crystal'] === true && $check['deuterium'] === true) {
                ...
                if (is_null($force)) {
                    foreach ($hangar as $k => $v) {
                        if ($check[$k] === false) {
                            return sprintf($this->_lang['tr_full_storage'],
                                strtolower($this->_lang['info'][$v]['name']));
                        } else {
                            continue;
                        }
                    }
