...
class Database extends XGPCore
{
    ...
    {
        $parse = $this->_lang;
        $template = parent::$page->getTemplate('adm/data_row_view');
        $parse['alert'] = '';
        ...
        if (!$_POST) {
            ...
            while ($row = parent::$db->fetchArray($Tablas)) {
                ...
                $parse['tabla'] .= parent::$page->parseTemplate($template, $row);
            }
        } else {
            $Tablas = parent::$db->query("SHOW TABLES", 'todas');
            $parse['tabla'] = '';
        ...
        parent::$page->display(parent::$page->parseTemplate(parent::$page->getTemplate('adm/data_view'), $parse));
    }
}
